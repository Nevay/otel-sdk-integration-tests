...
YAML,
     ...
     */
    private function spansInExport(string $payload): array
    {
        return $this->path(
            $payload,
     ...
     * @return list<array<string, mixed>>
     */
    private function logsInExport(string $payload): array
    {
        return $this->path(
            $payload,
     ...
     * @return list<string>
     */
    private function spanNames(string $payload): array
    {
        return $this->path(
            $payload,
    ...
    private function resourceAttribute(
        ...
        $values = $this->path(
            $payload,
            sprintf(
                '$.resourceSpans[*].resource.attributes[?(@.key == "%s")].value.stringValue',
                $name,
            ),
        );
     ...
     * @return list<array<string, mixed>>
     */
    private function spanAttributes(string $spanName): array
    {
        return $this->path(
    ...
    private function spanAttribute(
        string $spanName,
        string $attribute,
    ): mixed {
        $values = $this->path(
            $this->combinedTracePayload(),
            sprintf(
                '$.resourceSpans[*].scopeSpans[*].spans[?(@.name == "%s")].attributes[?(@.key == "%s")].value.stringValue',
                $spanName,
                $attribute,
            ),
        );
    ...
    private function combinedTracePayload(): string
    {
        $resourceSpans = [];
